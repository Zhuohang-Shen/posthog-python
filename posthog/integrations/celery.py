...
(started, success, failure, retry, revoked, internal error, published) and
...
class PosthogCeleryIntegration:
    ...
    Event properties:
        ...
        - ``celery_task_duration_ms`` -- task wall-clock duration in milliseconds
          (present on terminal states: success, failure, revoked, internal error, retry)

        Additional properties on specific states:

        - **failure**: ``error_type``, ``error_message``
        - **revoked**: ``celery_terminated``, ``celery_signum``, ``celery_expired``
        - **retry**: ``celery_reason``
    """
    ...
    def instrument(self) -> None:
        ...
        signals.task_revoked.connect(self._on_task_revoked, weak=False)
        signals.task_internal_error.connect(self._on_task_internal_error, weak=False)
    ...
    def uninstrument(self) -> None:
        ...
        self._signals.task_revoked.disconnect(self._on_task_revoked)
        self._signals.task_internal_error.disconnect(self._on_task_internal_error)
    ...
    def _on_after_task_publish(self, *args, **kwargs):
        try:
            ...
            sender = kwargs.get("sender")   # task name as this is executed in the parent process
            headers = kwargs.get("headers")
    ...
    def _on_task_revoked(self, *args, **kwargs):
        self._handle_task_end("revoked", extra_properties={
            "celery_terminated": kwargs.get("terminated"),
            "celery_signum": kwargs.get("signum"),
            "celery_expired": kwargs.get("expired"),
        }, **kwargs)
    ...
    def _on_task_internal_error(self, *args, **kwargs):
        self._handle_task_end("internal_error", **kwargs)
