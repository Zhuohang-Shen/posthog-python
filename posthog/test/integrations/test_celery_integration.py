...
class TestPosthogCeleryIntegration(unittest.TestCase):
    def test_instrument_is_idempotent(self):
        fake_signals = SimpleNamespace(
            ...
            task_revoked=FakeSignal(),
            task_internal_error=FakeSignal(),
    ...
    def test_instrument_and_uninstrument_connect_signals(self):
        ...
        for sig in ["task_prerun", "task_success", "task_failure", "task_revoked",
                    "task_internal_error", "task_retry", "before_task_publish",
                    "after_task_publish"]:
    ...
    def test_task_revoked_cleans_up_context_and_emits_event(self):
        mock_client = Mock()
        integration = PosthogCeleryIntegration(client=mock_client)
        ...
        task = SimpleNamespace(name="app.tasks.revokable", request=request)

        integration._on_task_prerun(sender=task, task_id="task-r")
        self.assertTrue(hasattr(request, "_posthog_ctx"))

        integration._on_task_revoked(
            sender=task, request=request, terminated=True, expired=False
        )

        revoked_calls = [
            c for c in mock_client.capture.call_args_list if c.args[0] == "celery task revoked"
        ]
        self.assertEqual(len(revoked_calls), 1)
        props = revoked_calls[0].kwargs["properties"]
        self.assertEqual(props["celery_state"], "revoked")
        self.assertTrue(props["celery_terminated"])
        self.assertIn("celery_task_duration_ms", props)

    def test_task_internal_error_captures_exception_and_event(self):
        mock_client = Mock()
        integration = PosthogCeleryIntegration(client=mock_client)

        task = SimpleNamespace(name="app.tasks.broken", request=SimpleNamespace(delivery_info={}))
        exception = RuntimeError("internal failure")

        integration._on_task_internal_error(
            sender=task,
            task_id="task-ie",
            exception=exception,
        )
        ...
        mock_client.capture_exception.assert_called_once_with(exception)
        event_names = [call.args[0] for call in mock_client.capture.call_args_list]
        self.assertIn("celery task internal_error", event_names)
